...
public:
   ConversionStep(CastKind Kind, QualType ty)
       : Kind(Kind), Halt(false), Data(ty.getAsOpaquePtr())
   {
   }

   explicit ConversionStep(ast::CallableDecl* ConvOp)
       : Kind(CastKind::ConversionOp), Halt(false), Data(ConvOp)
   {
   }

   ConversionStep() : Kind(CastKind::NoOp), Halt(true), Data{} {}
...
public:
   ...
   ConversionSequenceBuilder& operator=(const ConversionSequenceBuilder&)
       = delete;
   ...
   void addHalt() { Steps.emplace_back(); }
...
class ConversionSequence final
    : TrailingObjects<ConversionSequence, ConversionStep> {
...
public:
   ...
   static ConversionSequence* Create(ast::ASTContext& C, CastStrength Strength,
