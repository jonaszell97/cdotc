...
public:
   ...
   struct PathElement {
      ...
      static void Profile(llvm::FoldingSetNodeID& ID, LocatorKind Kind,
                          uint64_t Data);
      ...
      {
         return PathElement(OverloadedDeclName,
                            (uint64_t)Name.getAsOpaquePtr());
      }
   ...
   /// FoldingSetNode Impl
   void Profile(llvm::FoldingSetNodeID& ID) const;
   static void Profile(llvm::FoldingSetNodeID& ID, ast::Expression* Anchor,
...
private:
   ConstraintLocator(ast::Expression* Anchor, ArrayRef<PathElement> Elements);
...
protected:
   TypePropertyConstraint(ConstraintKind K, TypeVariable Var, Locator Loc);
...
public:
   static TypeBindingConstraint* Create(ConstraintSystem& Sys, TypeVariable Var,
...
public:
   static TypeInequalityConstraint*
   Create(ConstraintSystem& Sys, TypeVariable Var, QualType Type, Locator Loc);
...
public:
   static FunctionReturnTypeConstraint*
   Create(ConstraintSystem& Sys, TypeVariable Var, QualType Type, Locator Loc);
...
class CovarianceConstraint : public RelationalConstraint {
   CovarianceConstraint(TypeVariable Var, QualType Covariance, bool Variadic,
                        Locator Loc);
...
public:
   static CovarianceConstraint* Create(ConstraintSystem& Sys, TypeVariable Var,
...
class InferenceConstraint : public RelationalConstraint {
   InferenceConstraint(TypeVariable Var, QualType RHS, Locator Loc);

public:
   static InferenceConstraint* Create(ConstraintSystem& Sys, TypeVariable Var,
...
public:
   static DefaultableConstraint* Create(ConstraintSystem& Sys, TypeVariable Var,
                                        QualType Type, Locator Loc);
...
public:
   static ConformanceConstraint* Create(ConstraintSystem& Sys, TypeVariable Var,
                                        ast::ProtocolDecl* ProtoDecl,
                                        Locator Loc);
...
public:
   static ClassConstraint* Create(ConstraintSystem& Sys, TypeVariable Var,
                                  Locator Loc);

   /// isa<>, cast<> dyn_cast<>
   static bool classof(const Constraint* C) { return C->getKind() == ClassID; }
...
public:
   static MemberConstraint* Create(ConstraintSystem& Sys, TypeVariable Var,
                                   DeclarationName MemberName,
                                   QualType MemberType, Locator Loc);

   /// isa<>, cast<> dyn_cast<>
   static bool classof(const Constraint* C) { return C->getKind() == MemberID; }
...
class DisjunctionConstraint final
    : public Constraint,
...
class ConditionalConstraint final
    : public Constraint,
...
struct OverloadChoice {
   OverloadChoice(unsigned ChosenIndex) : ChosenIndex(ChosenIndex) {}
...
public:
   ...
   struct SolutionBindings {
      ...
      llvm::DenseMap<ast::OverloadedDeclRefExpr*, OverloadChoice>
          OverloadChoices;
   };
...
public:
   ...
   explicit ConstraintSystem(QueryContext& QC, SourceRange Loc = SourceRange(),
                             llvm::raw_ostream* LogStream = nullptr);
   ...
   template<class T, class... Args> T* newConstraint(Args&&... args)
   {
   ...
   template<class T> T* getFirstConstraint(TypeVariableType* TypeVar)
   {
...
inline void operator delete[](void* ptr,
                              const cdot::sema::ConstraintSystem& Sys, size_t)
{
   return Sys.Deallocate(ptr);
}
