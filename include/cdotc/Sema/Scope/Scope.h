...
#define CDOT_SCOPE(Name) Name##ID,
...
public:
   explicit BlockScope(unsigned ScopeID, ast::CompoundStmt* CS = nullptr,
                       Scope* enclosingScope = nullptr)
       : Scope(BlockScopeID, enclosingScope), ScopeID(ScopeID), CS(CS)
   {
   }
...
public:
   explicit FunctionScope(ast::CallableDecl* CD,
                          ...
                          Scope* enclosingScope = nullptr)
       : Scope(FunctionScopeID, enclosingScope), CD(CD),
         InferrableReturnType(inferrableReturnType)
   {
   }

   ast::CallableDecl* getCallableDecl();

   bool hasInferrableReturnType() const { return InferrableReturnType; }
...
protected:
   FunctionScope(TypeID typeID, ast::CallableDecl* CD,
                 bool inferrableReturnType = false,
                 Scope* enclosingScope = nullptr)
       : Scope(typeID, enclosingScope), CD(CD),
         InferrableReturnType(inferrableReturnType)
   {
   }
...
public:
   explicit MethodScope(ast::MethodDecl* M, bool InferrableReturnType = false,
                        Scope* enclosingScope = nullptr);
...
public:
   ...
   ast::LambdaExpr* getLambdaExpr() const { return L; }
...
public:
   LoopScope(bool continuable, bool breakable, bool lastCaseInMatch,
             bool nextCaseHasArgs, Scope* enclosingScope = nullptr)
       : Scope(LoopScopeID, enclosingScope), continuable(continuable),
         breakable(breakable), lastCaseInMatch(lastCaseInMatch),
...
public:
   StaticForScope(IdentifierInfo* elementName, QualType elementTy,
                  Scope* enclosingScope = nullptr);
