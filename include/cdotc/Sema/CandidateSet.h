...
struct CandidateSet {
   ...
   struct Candidate {
      ...
      bool isAnonymousCandidate() const { return IsAnonymousCand; }
      ...
      void setHasIncompatibleTemplateArgKind(unsigned diagSelect, size_t idx)
      {
      ...
      void setMutatingOnRValueSelf() { FR = MutatingOnRValueSelf; }

      void setMustBeStatic() { FR = MustBeStatic; }
      ...
      void setIsInvalid() { FR = IsInvalid; }

      void setIsDependent() { FR = IsDependent; }
   };
   ...
   bool isAmbiguous() const { return Status == Ambiguous; }

   bool noMatchFound() const { return Status == NoMatch; }

   operator bool() const { return !isAmbiguous() && !noMatchFound(); }

   void diagnose(ast::SemaPass& SP, ast::Expression* SelfVal,
                 ArrayRef<ast::Expression*> args,
                 ArrayRef<ast::Expression*> templateArgs,
                 ast::Statement* Caller, SourceLocation OpLoc = {})
   {
      if (Status == NoMatch)
   ...
   void diagnoseFailedCandidates(ast::SemaPass& SP, ast::Expression* SelfVal,
                                 ArrayRef<ast::Expression*> args,
                                 ArrayRef<ast::Expression*> templateArgs,
                                 ast::Statement* Caller,
