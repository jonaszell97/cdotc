...
private:
   ...
   struct UnfinishedBasicBlock {
      UnfinishedBasicBlock(il::BasicBlock& B, SmallVector<unsigned, 4>&& ArgIDs,
   ...
   struct FunctionRAII {
      ...
      ~FunctionRAII() { Reader.CurrentFn = F; }
...
public:
   ...
   struct EagerRAII {
      ...
      ~EagerRAII() { R.ReadGlobals = Previous; }
...
public:
   ...
   /// Reads a value
   il::Value* readValue() { return Reader->readValue(Record, Idx); }

   unsigned readValueID() { return Reader->ReadValueID(Record, Idx); }

   /// Reads a value as a specific type
   template<class T> T* readValueAs()
   {
...
public:
   ...
   ILSymbolTableLookupTrait(ILReader& Reader) : Reader(Reader) {}
