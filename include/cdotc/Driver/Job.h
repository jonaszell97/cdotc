...
protected:
   Job(Kind K, Job* PreviousJob, CompilerInstance& CI)
       : K(K), Done(false), HadError(false), PreviousJob(PreviousJob), CI(CI)
   {
   }
...
public:
   ...
   {
      return J->getKind() == ILCanonicalizeJobID;
   }
};
...
public:
   explicit EmitAssemblyJob(StringRef OutFile, Job* PreviousJob,
                            CompilerInstance& CI);
...
public:
   explicit EmitObjectJob(StringRef OutFile, Job* PreviousJob,
                          CompilerInstance& CI);
...
public:
   explicit EmitStaticLibraryJob(StringRef OutFile, Job* PreviousJob,
                                 CompilerInstance& CI);
...
public:
   explicit EmitDynamicLibraryJob(StringRef OutFile, Job* PreviousJob,
                                  CompilerInstance& CI);
...
public:
   explicit EmitExecutableJob(StringRef OutFile, Job* PreviousJob,
                              CompilerInstance& CI);
...
public:
   explicit EmitModuleJob(Module& Mod, CompilerInstance& CI);
...
public:
   explicit EmitILJob(il::Module& M, StringRef OutFile, CompilerInstance& CI);
...
public:
   explicit EmitIRJob(Job* PreviousJob, StringRef OutFile,
                      CompilerInstance& CI);
...
public:
   LinkJob(ArrayRef<Job*> Inputs, StringRef OutFile, CompilerInstance& CI);
...
public:
   LinkIRJob(ArrayRef<Job*> Inputs, CompilerInstance& CI);
...
public:
   ...
   static bool classof(const Job* J)
   {
      return J->getKind() == PrintPhasesJobID;
   }
};
