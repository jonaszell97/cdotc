...
public:
   ...
   llvm::StringRef getIdentifier() const { return Ident; }

   lex::tok::TokenType getKeywordTokenKind() const { return keywordTokenKind; }
   ...
   unsigned getLength() const { return (unsigned)Ident.size(); }

   const char* getNameStart() const { return Ident.data(); }

private:
   IdentifierInfo() : keywordTokenKind(lex::tok::sentinel) {}
...
public:
   ...
   IdentifierTable(unsigned initialSize = 8192) : IdentMap(initialSize) {}
   ...
   AllocatorTy& getAllocator() { return IdentMap.getAllocator(); }
