...
inline int operator>(const DeclarationName& LHS, const DeclarationName& RHS)
{
...
template<> struct DenseMapInfo<cdot::DeclarationName> {
...
template<> struct PointerLikeTypeTraits< ::cdot::DeclarationName> {
public:
...
public:
   ...
   DeclarationName getPackExpansionName(DeclarationName Name, unsigned idx);
   ...
   DeclarationName getLocalVarName(DeclarationName Name, unsigned ScopeID);
   ...
   DeclarationName
   getInstantiationName(DeclarationName BaseName,
...
llvm::raw_ostream& operator<<(llvm::raw_ostream& OS, NestedNameSpecifier* Name);
