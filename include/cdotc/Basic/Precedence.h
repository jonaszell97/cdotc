...
enum class Associativity : unsigned char { Left, Right };
...
public:
   ...
   bool isValid() const { return precedence != -1; }
...
public:
   ...
   OperatorInfo(const PrecedenceGroup& precedenceGroup, FixKind fix,
                bool implicit = false)
       : precedenceGroup(precedenceGroup), fix(fix), implicit(implicit)
   {
   }

   const PrecedenceGroup& getPrecedenceGroup() const { return precedenceGroup; }

   FixKind getFix() const { return fix; }
   ...
   void setFix(FixKind fix) { OperatorInfo::fix = fix; }

   bool isImplicit() const { return implicit; }

   void setImplicit(bool implicit) { OperatorInfo::implicit = implicit; }
...
#define CDOT_PRECEDENCE_GROUP(Name, Prec, Assoc) Name = Prec,
...
#define CDOT_OPERATOR(Name, Symbol, PG, Fix) Name,
#include "cdotc/Basic/BuiltinOperators.def"
};
