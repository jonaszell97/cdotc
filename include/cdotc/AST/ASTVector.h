...
#ifndef CDOT_ASTVECTOR_H
#define CDOT_ASTVECTOR_H
...
public:
   ...
   ASTVector(const ast::ASTContext& C, unsigned N) : Capacity(nullptr, false)
   {
      reserve(C, N);
   }

   ASTVector(const ast::ASTContext& C, llvm::ArrayRef<T> Arr)
       : Capacity(nullptr, false)
   {
      reserve(C, Arr.size());
      insert(C, data(), Arr.begin(), Arr.end());
   }
   ...
   void push_back(const_reference Elt, const ast::ASTContext& C)
   {
   ...
   void reserve(const ast::ASTContext& C, unsigned N)
   {
   ...
   void append(const ast::ASTContext& C, in_iter in_start, in_iter in_end)
   {
      ...
      // access iterator to use the fast uninitialized_copy.
      std::uninitialized_copy(in_start, in_end, this->end());
      this->setEnd(this->end() + NumInputs);
   }

   /// append - Add the specified range to the end of the SmallVector.
   void append(const ast::ASTContext& C, size_type NumInputs, const T& Elt)
   {
      // Grow allocated space if needed.
      if (NumInputs > size_type(this->capacity_ptr() - this->end()))
         this->grow(C, this->size() + NumInputs);
   ...
   iterator insert(const ast::ASTContext& C, iterator I, const T& Elt)
   {
   ...
   iterator insert(const ast::ASTContext& C, iterator I, size_type NumToInsert,
                   const T& Elt)
   {
   ...
   iterator insert(const ast::ASTContext& C, iterator I, ItTy From, ItTy To)
   {
      // Convert iterator to elt# to avoid invalidating iterator when we
      // reserve()
      size_t InsertElt = I - this->begin();
   ...
   void resize(const ast::ASTContext& C, unsigned N, const T& NV)
   {
      if (N < this->size()) {
   ...
   operator llvm::ArrayRef<T>() const { return {begin(), size()}; }
   operator llvm::MutableArrayRef<T>() { return {begin(), size()}; }

private:
   /// grow - double the size of the allocated memory, guaranteeing space for at
   /// least one more element or MinSize if specified.
   void grow(const ast::ASTContext& C, size_type MinSize = 1);
...
template<typename T>
void ASTVector<T>::grow(const ast::ASTContext& C, size_t MinSize)
{
