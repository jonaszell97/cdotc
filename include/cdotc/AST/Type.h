...
template<> struct PointerLikeTypeTraits<::cdot::Type*> {
public:
...
class Qualifiers {
public:
   explicit Qualifiers(unsigned Quals) : Quals(Quals) { assert(Quals == 0); }
...
#define CDOT_TYPE(Name, Parent) class Name;
...
public:
   ...
   TypeProperties& operator|=(Property P)
   {
      Props |= P;
      return *this;
   }
...
#define CDOT_TYPE(Name, Parent) Name##ID,
#include "cdotc/AST/Types.def"
   };
...
#define CDOT_BUILTIN_TYPE(Name) Name,
...
#include "cdotc/AST/Types.def"
   ...
   bool isInt64Ty() const { return isIntegerType() && getBitwidth() == 64; }

   bool isInt8Ty() const { return isIntegerType() && getBitwidth() == 8; }

   bool isInt1Ty() const { return isIntegerType() && getBitwidth() == 1; }
...
protected:
   ...
   enum class FromOpaque { Placeholder };

   QualType(void* Ptr, FromOpaque) : Value(ValueTy::getFromOpaqueValue(Ptr)) {}

public:
   QualType() : Value(nullptr) {}

   QualType(Type* ty) : Value(ty) {}

   QualType(Type* ty, Qualifiers quals) : Value(ty, quals.getRaw())
   {
      assert(quals.getRaw() == 0);
   }

   QualType(Type* ty, unsigned quals) : Value(ty, quals) { assert(quals == 0); }
   ...
   bool isCanonical() const { return (**this)->isCanonical(); }
   ...
   void Profile(llvm::FoldingSetNodeID& ID) { ID.AddPointer(getAsOpaquePtr()); }
...
inline CanType Type::getCanonicalType() const { return CanType(CanonicalType); }
...
template<> struct simplify_type<::cdot::QualType> {
   ...
   static SimpleType getSimplifiedValue(::cdot::QualType Val) { return *Val; }
};
...
template<> struct simplify_type<::cdot::CanType> {
   ...
   static SimpleType getSimplifiedValue(::cdot::CanType Val) { return *Val; }
};

template<> struct PointerLikeTypeTraits<::cdot::QualType> {
public:
...
template<> struct PointerLikeTypeTraits<::cdot::CanType> {
public:
...
template<> struct DenseMapInfo<::cdot::QualType> {
   ...
   static bool isEqual(const ::cdot::QualType& LHS, const ::cdot::QualType& RHS)
   {
      return LHS == RHS;
   }
};
...
template<> struct DenseMapInfo<::cdot::CanType> {
   ...
   static bool isEqual(const ::cdot::CanType& LHS, const ::cdot::CanType& RHS)
   {
      return LHS == RHS;
   }
};
...
public:
   ...
   unsigned short getAlignment() const { return alignof(void*); }
...
protected:
   MutableReferenceType(TypeID ID, QualType referencedType,
                        Type* CanonicalType);
};
...
public:
   ...
   size_t getSize() const { return elementType->getSize() * numElements; }

   unsigned short getAlignment() const { return elementType->getAlignment(); }
...
public:
   ast::StaticExpr* getSizeExpr() const { return DependentExpr; }
...
private:
   FunctionType(QualType returnType, llvm::ArrayRef<QualType> argTypes,
                llvm::ArrayRef<ParamInfo> paramInfo, ExtFlags flags,
                Type* CanonicalType, TypeProperties Props);

protected:
   FunctionType(TypeID typeID, QualType returnType,
...
public:
   ...
   iterator param_end() const { return param_begin() + NumParams; }
   ...
   unsigned short getAlignment() const { return sizeof(void*); }
...
class LambdaType : public FunctionType {
   LambdaType(QualType returnType, llvm::ArrayRef<QualType> argTypes,
              llvm::ArrayRef<ParamInfo> paramInfo, ExtFlags flags,
              Type* CanonicalType, TypeProperties Props);
...
class TupleType final : public Type, public llvm::FoldingSetNode {
   TupleType(llvm::ArrayRef<QualType> containedTypes, Type* CanonicalType,
             TypeProperties Props);
...
protected:
   ...
   RecordType(TypeID typeID, ast::RecordDecl* record, bool Dependent);
...
public:
   ...
   void Profile(llvm::FoldingSetNodeID& ID) { Profile(ID, getRecord()); }
...
public:
   ...
   void Profile(llvm::FoldingSetNodeID& ID) { Profile(ID, P); }
...
public:
   ...
   void Profile(llvm::FoldingSetNodeID& ID) { Profile(ID, AT, OuterAT); }
...
protected:
   TypedefType(TypeID typeID, ast::AliasDecl* td, bool Dependent);
...
public:
   ...
   void Profile(llvm::FoldingSetNodeID& ID) { Profile(ID, getTypedef()); }
