...
struct SourceType {
   explicit SourceType(Expression* TypeExpr) : TypeExpr(TypeExpr) {}
   ...
   bool isResolved() const { return !ResolvedType.isNull(); }

   Type* operator->() const { return *ResolvedType; }
...
template<> struct PointerLikeTypeTraits<::cdot::ast::SourceType> {
public:
...
template<> struct DenseMapInfo<::cdot::ast::SourceType> {
   static ::cdot::ast::SourceType getEmptyKey()
   {
      ...
      return ::cdot::ast::SourceType(
          ::cdot::QualType::getFromOpaquePtr((void*)Val));
   }
