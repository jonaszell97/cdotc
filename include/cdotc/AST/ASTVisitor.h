...
template<class SubClass, class ExprRetTy = void, class StmtRetTy = ExprRetTy,
         class DeclRetTy = void, class... ParamTys>
class ASTVisitor {
public:
...
#define CDOT_DECL(Name)                                                        \
   case Decl::Name##ID:                                                        \
      return static_cast<SubClass*>(this)->visit##Name(                        \
          static_cast<Name*>(D), std::forward<ParamTys>(params)...);
#include "cdotc/AST/Decl.def"
...
template<class SubClass, class... ParamTys> class RecursiveASTVisitor {
public:
...
protected:
   ...
   bool visitFunctionTypeExpr(FunctionTypeExpr* Expr)
   {
      ...
      static_cast<SubClass*>(this)->visitStmt(
          Expr->getReturnType().getTypeExpr());
      return true;
   }
...
class SpecificASTVisitor
    : public RecursiveASTVisitor<SpecificASTVisitor<CallbackFn, StmtClass...>> {
   CallbackFn fn;

   template<class T> void tryVisit(Statement* stmt)
   {
...
public:
   explicit SpecificASTVisitor(CallbackFn&& fn) : fn(std::move(fn)) {}

   bool visitStmt(Statement* stmt)
   {
      (tryVisit<StmtClass>(stmt), ...);
      return RecursiveASTVisitor<
          SpecificASTVisitor<CallbackFn, StmtClass...>>::visit(stmt);
   }
};
...
template<class SubClass> class StmtBuilder {
protected:
   StmtBuilder() = default;
   ...
   StmtResult visitBreakStmt(BreakStmt* Stmt) { return Stmt; }

   StmtResult visitContinueStmt(ContinueStmt* Stmt) { return Stmt; }
   ...
   StmtResult visitDebugStmt(DebugStmt* Stmt) { return Stmt; }

   StmtResult visitNullStmt(NullStmt* Stmt) { return Stmt; }
   ...
   StmtResult visitMacroExpansionStmt(MacroExpansionStmt* Stmt) { return Stmt; }

   StmtResult visitDeclStmt(DeclStmt* Stmt) { return Stmt; }
   ...
   ExprResult visitConstraintExpr(ConstraintExpr* Expr) { return Expr; }
   ...
   ExprResult visitDeclTypeExpr(DeclTypeExpr* Expr) { return Expr; }
...
template<class SubClass> class ASTTypeVisitor {
public:
...
#include "cdotc/AST/Decl.def"
   ...
   void visitCompoundDecl(CompoundDecl* D) { visitDeclContext(D); }
   ...
   void visitPropDecl(PropDecl* D) { visitType(D->getType()); }

   void visitSubscriptDecl(SubscriptDecl* D) { visitType(D->getType()); }

   void visitTypedefDecl(TypedefDecl* D) { llvm_unreachable("NO!"); }

   void visitNamespaceDecl(NamespaceDecl* D) { visitDeclContext(D); }
   ...
   void visitModuleDecl(ModuleDecl* D) { visitDeclContext(D); }
   ...
   void visitGlobalVarDecl(GlobalVarDecl* D) { visitVarDecl(D); }
   ...
   void visitFuncArgDecl(FuncArgDecl* D) { visitVarDecl(D); }

   void visitFieldDecl(FieldDecl* D) { visitVarDecl(D); }
   ...
   void visitStructDecl(StructDecl* D) { visitRecordDecl(D); }
   ...
   void visitUnionDecl(UnionDecl* D) { visitRecordDecl(D); }

   void visitProtocolDecl(ProtocolDecl* D) { visitRecordDecl(D); }
   ...
   void visitFunctionDecl(FunctionDecl* D) { visitCallableDecl(D); }

   void visitMethodDecl(MethodDecl* D) { visitCallableDecl(D); }

   void visitInitDecl(InitDecl* D) { visitMethodDecl(D); }

   void visitDeinitDecl(DeinitDecl* D) { visitMethodDecl(D); }

   void visitEnumCaseDecl(EnumCaseDecl* D) { visitCallableDecl(D); }
};
