...
class ArrayView {
public:
   ArrayView(llvm::ArrayRef<Orig> arr) : arr(arr) {}
...
public:
   ...
   public:
      ...
      iterator(Orig const* Ptr) : Ptr(Ptr) {}

   private:
      Orig const* Ptr;

   public:
      Dest operator*() const { return Projector(*Ptr); }

      Dest operator->() const { return operator*(); }
...
public:
   ...
   ArrayView slice(unsigned start) const { return ArrayView(arr.slice(start)); }
...
public:
   FilteredArrayView(llvm::ArrayRef<Orig*> arr) : arr(arr) {}
...
public:
   ...
   public:
      ...
      iterator(Orig* const* Ptr, Orig* const* End) : Ptr(Ptr), End(End)
      {
         advanceUntilNotNull();
      }
