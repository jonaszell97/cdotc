...
void formatInteger(uint64_t val, llvm::SmallString<N>& res)
{
   ...
   auto neededDigits
       = uint64_t(std::floor(std::log(val) / std::log(FormatTraits::Base))) + 1;
...
template<class Stream> inline Stream& unescape_char(char c, Stream& out)
{
   switch (c) {
   case '\n':
      return out << "\\n";
   ...
   default:
      if (!::isprint(c)) {
         return out << '\\' << hexdigit((c & 0b11110000) >> 4)
                    << hexdigit(c & 0b1111);
      }
      else
         return out << c;
