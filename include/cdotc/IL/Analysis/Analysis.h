...
public:
   explicit AnalysisPreserver(Analysis* A) : A(A) { A->lockInvalidation(); }

   ~AnalysisPreserver() { A->unlockInvalidation(); }
};

/// An abstract base class that implements the boiler plate of caching and
/// invalidating analysis for specific functions.
template<typename AnalysisTy> class FunctionAnalysisBase : public Analysis {
protected:
...
public:
   ...
   /// Invalidate all information in this analysis.
   void invalidate() override { deleteAllAnalysisProviders(); }
   ...
   /// Notify the analysis about a function which will be deleted from the
   /// module.
   void notifyDeleteFunction(Function* F) override { invalidateFunction(F); }

   explicit FunctionAnalysisBase(AnalysisKind Kind) : Analysis(Kind), Storage()
   {
   }

   ~FunctionAnalysisBase() override { deleteAllAnalysisProviders(); }
...
template<class AnalysisTy, class FunctionInfoTy> class LazyFunctionInfo {
   Function* F;
   AnalysisTy* A;
   FunctionInfoTy* FTy;
