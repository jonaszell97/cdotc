...
class BottomUpIPAnalysis : public Analysis {
...
protected:
   ...
   public:
      ...
      struct CallerEntry {
         ...
         /// Returns false, if the caller was invalidated since this entry had
         /// been created.
      ...
      private:
         ...
         CallerEntry(FunctionInfo* Caller, Instruction* FAS, int UpdateID)
   ...
   private:
      ...
      /// invalidated. Note that the list may contain invalid entries for
      /// already invalidated callers. Those entries are removed lazily in
      /// removeInvalidCallers().
      /// The lazy removal of invalid entries avoids that we additionally need
      /// to store a callee-set. Also it reduces the number of times we need to
      /// iterate through the Callers (in case multiple caller functions are
      /// invalidated at the same time).
      /// Note that Callers can't be a pure hashed set-like container because
      ...
      enum { NotVisited = -2, Visited = -1, FirstPosition = 0 };
      ...
      void removeInvalidCallers()
      {
         Callers.erase(
             std::remove_if(Callers.begin(), Callers.end(),
                            [](const CallerEntry& E) { return !E.isValid(); }),
             Callers.end());
      }
   ...
   public:
      ...
      /// Returns true if this function was already visited during
      /// recomputation.
      ...
      /// Returns true if this function is scheduled after \p RHS in the
      /// bottom-up order, i.e. is considered as a "caller" of \p RHS.
      ...
      llvm::ArrayRef<CallerEntry> getCallers() const { return Callers; }

      /// Adds a caller for this function.
      void addCaller(FunctionInfo* CallerInfo, Instruction* FAS)
      {
      ...
      void addCaller(FunctionInfo* CallerInfo, CallSite CS)
      {
         addCaller(CallerInfo, CS.getInstruction());
      }
   };
   ...
   public:
      ...
      {
         assert(InitiallyUnscheduled.empty() && "not finished scheduling");
         ...
         for (FunctionInfo* FInfo : Scheduled) {
            ...
            FInfo->StateAndPosition
                = FunctionInfoBase<FunctionInfo>::NotVisited;
         }
      }
      ...
      {
         assert(!FInfo->isVisited() && "function visited multiple times");
         assert(FInfo->numUnscheduledCallees == 0
      ...
      void tryToSchedule(FunctionInfo* FInfo)
      {
         ...
         assert(!FInfo->isScheduled() && "function scheduled multiple times");
   ...
   BottomUpIPAnalysis(AnalysisKind K) : Analysis(K) {}
   ...
   /// Invalidates \p FInfo, including all analysis data which depend on it,
   /// i.e. the callers.
...
#endif // CDOT_BOTTOMUPIPANALYSIS_H
