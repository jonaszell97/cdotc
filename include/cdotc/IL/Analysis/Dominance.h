...
public:
   ...
   inline bool errorOccurredOnComparison(const PostDominanceInfo& Other) const
   {
      ...
      if (!R->getBlock()) {
         ...
         // an infinite loop in the CFG (it crashes because no nodes are
         // allocated for the blocks in the infinite loop). So we return a
         // conservative false in this case.
...
public:
   PostDominanceAnalysis()
       : FunctionAnalysisBase<PostDominanceInfo>(
             AnalysisKind::PostDominanceAnalysisID)
   {
   }
