...
#include "cdotc/IL/SymbolTableList.h"
...
public:
   ...
   explicit Module(Context& Ctx, size_t fileID = 0,
                   llvm::StringRef fileName = {}, llvm::StringRef path = {});

   ~Module();

   Function* insertFunction(Function* Fn, bool OverridePrevious = false,
   ...
   void writeTo(llvm::raw_ostream& out,
                NameProvider* nameProvider = nullptr) const;
   void writeToFile(const char* FileName) const;

   bool
   linkInModule(std::unique_ptr<Module>&& M,
   ...
   bool hasExternallyVisibleSymbols() const
   {
      return HasExternallyVisibleSymbols;
   }
   void setHasExternallyVisibleSymbols(bool V)
   {
      HasExternallyVisibleSymbols = V;
   }

   bool containsDeserializedValues() const
   {
      return ContainsDeserializedValues;
   }
   void setContainsDeserializedValues(bool V)
   {
      ContainsDeserializedValues = V;
   }
