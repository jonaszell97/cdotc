...
public:
   ...
   void invalidateAllAnalyses()
   {
      ...
      // Assume that all functions have changed. Clear all masks of all
      // functions.
   ...
   void invalidateAnalysis(Function* F, Analysis::InvalidationKind K)
   {
      // Invalidate the analysis (unless they are locked)
      for (auto AP : Analyses)
         if (!AP->isLocked())
