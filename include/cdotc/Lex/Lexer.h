...
public:
   ...
   Lexer(IdentifierTable& Idents, DiagnosticsEngine& Diags,
         llvm::MemoryBuffer* buf, unsigned sourceId, unsigned offset = 1,
         const char InterpolationBegin = '$', bool primeLexer = true);

   Lexer(IdentifierTable& Idents, DiagnosticsEngine& Diags,
         llvm::ArrayRef<Token> Tokens, unsigned sourceId, unsigned offset = 1);
   ...
   struct ModeRAII {
      ModeRAII(Lexer& L, Mode M) : L(L), Prev(L.CurMode) { L.CurMode = M; }

      ~ModeRAII() { L.CurMode = Prev; }
   ...
   template<class... Rest> void expect(tok::TokenType ty, Rest... rest)
   {
      advance();
      expect_impl(ty, rest...);
   }

   template<class... Rest> void expect_impl(tok::TokenType ty, Rest... rest)
   {
      if (currentTok().is(ty))
         return;
      expect_impl(rest...);
   ...
   template<class... Rest> bool advanceIf(tok::TokenType ty, Rest... rest)
   {
      if (currentTok().is(ty)) {
         advance();
         return true;
...
protected:
   ...
   template<class... Args> Token makeToken(Args&&... args)
   {
