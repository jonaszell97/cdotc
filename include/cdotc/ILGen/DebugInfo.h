...
#include "cdotc/AST/AstDeclarations.h"
...
public:
   ...
   llvm::DIScope* getCurrentScope() { return ScopeStack.top(); }
   ...
   llvm::MDNode* emitFunctionDI(ast::FunctionDecl* node, llvm::Function* func);
   llvm::MDNode* emitLambdaDI(ast::LambdaExpr* node, llvm::Function* func);

   void emitParameterDI(const std::vector<ast::FuncArgDecl*>& args,
                        llvm::Function* func, unsigned int argBegin = 0,
                        bool emitSelf = false);

   llvm::DIScope* getFunctionScope(llvm::StringRef funcName);

   void beginGeneratedFunctionScope(llvm::StringRef funcName,
