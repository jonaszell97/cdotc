...
public:
   ...
   void visitPatternExpr(PatternExpr* E, il::Value* MatchVal,
                         il::BasicBlock* MatchBB, il::BasicBlock* NoMatchBB,
   ...
   il::Value* visitCasePattern(CasePattern* node, il::Value* MatchVal = nullptr,
                               il::BasicBlock* MatchBB = nullptr,
                               il::BasicBlock* NoMatchBB = nullptr,
                               ArrayRef<il::Value*> MatchArgs = {},
                               ArrayRef<il::Value*> NoMatchArgs = {});

   il::Value* visitIsPattern(IsPattern* node, il::Value* MatchVal = nullptr,
                             il::BasicBlock* MatchBB = nullptr,
                             il::BasicBlock* NoMatchBB = nullptr,
                             ArrayRef<il::Value*> MatchArgs = {},
                             ArrayRef<il::Value*> NoMatchArgs = {});
...
public:
   ...
   void SpecializeFunction(CallableDecl* Template, CallableDecl* Inst);

   void DefineLazyGlobal(il::GlobalVariable* G, Expression* defaultVal);
   ...
   makeArgVec(llvm::ArrayRef<QualType> from);

   il::Value* LookThroughLoad(il::Value* V);

public:
   il::Value* HandleCast(const ConversionSequence& res, il::Value* Val,
                         bool forced = false);

   il::Value* applySingleConversionStep(const ConversionStep& Step,
                                        il::Value* Val, bool forced = false);

   il::Value* doFunctionCast(ArrayRef<ConversionStep> Steps, il::Value* Val,
                             bool forced = false);

   il::Value* doFunctionCast(const ConversionSequence& ConvSeq, il::Value* Val,
                             bool forced = false);

   il::Value* doTupleCast(ArrayRef<ConversionStep> Steps, il::Value* Val,
                          bool forced = false);

   il::Value* doTupleCast(const ConversionSequence& ConvSeq, il::Value* Val,
                          bool forced = false);

   il::Value* Convert(il::Value* Val, QualType ToTy, bool forced = false);

   il::Value* DoPointerArith(op::OperatorKind op, il::Value* lhs,
                             il::Value* rhs);
   ...
   il::Value* CreateCall(CallableDecl* C, llvm::ArrayRef<il::Value*> args,
                         Expression* Caller = nullptr, bool DirectCall = false,
                         il::Value* GenericArgs = nullptr);

   il::Value* GetCallArgument(Expression* ArgExpr, CallableDecl* Fn,
                              unsigned ArgNo);
   ...
   il::Value* getCStyleArray(Type* Ty, llvm::ArrayRef<il::Value*> elements,
                             size_t minCapacity = 0, bool onHeap = false,
   ...
   il::Value*
   GetPotentiallyDynamicGenericArguments(ArrayRef<Expression*> ArgExprs,
   ...
   il::Value* GetGenericEnvironment(QualType ParentType, il::Value* Args);

   il::Value* GetGenericEnvironment(il::Value* EnclosingEnv,
                                    sema::FinalTemplateArgumentList* Args);

   il::Value* GetGenericEnvironment(il::Value* EnclosingEnv, il::Value* Args);
...
private:
   ...
   llvm::DenseMap<RecordDecl*,
                  llvm::DenseMap<ProtocolDecl*, il::GlobalVariable*>>
       PTableMap;
...
private:
   ...
   struct BreakContinueRAII {
      BreakContinueRAII(ILGenPass& ILGen, il::BasicBlock* BreakTarget,
                        il::BasicBlock* ContinueTarget = nullptr,
                        const CleanupsDepth& CleanupUntil = CleanupsDepth(),
                        IdentifierInfo* Label = nullptr);
...
private:
   ...
   struct EnterCtfeScope {
      EnterCtfeScope(ILGenPass& ILGen, CallableDecl* CalledFn) : ILGen(ILGen)
      {
...
public:
   struct CleanupRAII : public CleanupScope {
      CleanupRAII(ILGenPass& ILGen)
          : CleanupScope(ILGen.Cleanups), ILGen(ILGen),
            SavedCleanupScope(ILGen.LastCleanupScope)
      {
         ILGen.LastCleanupScope = this;
      }

      CleanupScope* getOuterCleanupScope() { return SavedCleanupScope; }

      ~CleanupRAII() { ILGen.LastCleanupScope = SavedCleanupScope; }
   ...
   struct ExprCleanupRAII : public ExprCleanupScope {
      ExprCleanupRAII(ILGenPass& ILGen)
          : ExprCleanupScope(ILGen.Cleanups), ILGen(ILGen),
            SavedCleanupScope(ILGen.LastCleanupScope)
      {
         ILGen.LastCleanupScope = this;
      }

      ~ExprCleanupRAII() { ILGen.LastCleanupScope = SavedCleanupScope; }
...
private:
   struct CompoundStmtRAII {
      ...
      ~CompoundStmtRAII() { ILGen.CompoundStmtStack.pop(); }
...
private:
   ...
   void registerReferencedGlobal(VarDecl* Decl, il::GlobalVariable* GV,
                                 Expression* RefExpr);
