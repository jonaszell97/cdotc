...
#undef INT_INIT
   ...
   static Value getBool(bool val) { return Value(uint64_t(val)); }

   static Value getDouble(double d) { return Value(d); }

   static Value getFloat(float f) { return Value(f); }

   static Value getPtr(void* ptr, Allocator& Alloc) { return Value(ptr); }

   static Value getConstPtr(void* ptr) { return Value(ptr); }
   ...
   static Value getPreallocated(void* buffer) { return Value(buffer); }
