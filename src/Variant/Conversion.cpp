            ...
            case LONG_T: {
                if (target_type == STRING_T) {
                    ...
                    if ((v.type.type == INT_T ? v.int_val : v.long_val) > UINT8_MAX) {
                        RuntimeError::raise(ERR_VAL_TOO_LARGE, "Value " + std::to_string(v.type.type == INT_T ? v.int_val : v.long_val)
                                                               + " is too large for type char");
                    }

                    v.c_val = char(v.type.type == INT_T ? v.int_val : v.long_val);
                }
                else if (target_type == BOOL_T) {
                    v.b_val = (v.type.type == INT_T ? v.int_val : v.long_val) != 0;
                }
                else if (target_type == FLOAT_T) {
                    v.float_val = (v.type.type == INT_T ? v.int_val : v.long_val);
                }
                else if (target_type == DOUBLE_T) {
                    v.d_val = (v.type.type == INT_T ? v.int_val : v.long_val);
                }
                else {
                    RuntimeError::raise(ERR_BAD_CAST, "Cannot cast value of type.type " + val::typetostr(v.type.type)
                                                      + " to " + val::typetostr(target_type));
                }
                ...
                if (target_type == STRING_T) {
                    ...
                    if ((v.type.type == FLOAT_T ? v.float_val : v.d_val) > UINT8_MAX) {
                        RuntimeError::raise(ERR_VAL_TOO_LARGE, "Value "
                                                               + std::to_string(v.type.type == FLOAT_T ? v.float_val : v.d_val)
                                                               + " is too large for type char");
                    }

                    v.c_val = char(v.type.type == FLOAT_T ? v.float_val : v.d_val);
                }
                else if (target_type == BOOL_T) {
                    v.b_val = (v.type.type == FLOAT_T ? v.float_val : v.d_val) != 0;
                }
                else if (target_type == FLOAT_T) {
                    v.float_val = (v.type.type == FLOAT_T ? v.float_val : float(v.d_val));
                }
                else if (target_type == INT_T) {
                    v.int_val = int(v.type.type == FLOAT_T ? v.float_val : v.d_val);
                }
                else if (target_type == LONG_T) {
                    v.long_val = long(v.type.type == FLOAT_T ? v.float_val : v.d_val);
                }
                else if (target_type == DOUBLE_T) {
                    v.d_val = (v.type.type == FLOAT_T ? v.float_val : v.d_val);
                }
                else {
                    RuntimeError::raise(ERR_BAD_CAST, "Cannot cast value of type " + val::typetostr(v.type.type)
                                                      + " to " + val::typetostr(target_type));
