...
namespace cdot {
    ...
    bool FPType::implicitlyCastableTo(Type *other) {
        switch (other->getTypeID()) {
            ...
            case TypeID::GenericTypeID: {
                ...
                auto clName = toString();

                if (asGen->getContravariance() == nullptr) {
                    cdot::cl::Class* cl = SymbolTable::getClass(clName);
                    ...
                    return cl->is_base_class_of(clName);
                }
            }
    ...
    llvm::Constant* FPType::getConstantVal(Variant &val) {
        return llvm::ConstantFP::get(getLlvmType(), val.getFloat());
    }
    ...
    string FPType::toString() {
        return precision == 64 ? "Double" : "Float" + Type::toString();
    }
