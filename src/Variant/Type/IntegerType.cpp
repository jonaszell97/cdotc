...
namespace cdot {
    ...
    bool IntegerType::implicitlyCastableTo(Type *other) {
        switch (other->getTypeID()) {
            ...
            case TypeID::GenericTypeID: {
                ...
                if (asGen->getContravariance() == nullptr) {
                    cdot::cl::Class* cl = SymbolTable::getClass(boxedCl);
                    return cl->is_base_class_of(asGen->getCovariance()->getClassName());
                }
                else {
                    cdot::cl::Class* cl = SymbolTable::getClass(asGen->getContravariance()->getClassName());
                    return cl->is_base_class_of(boxedCl);
                }
            }
    ...
    llvm::Constant* IntegerType::getConstantVal(Variant& val) {
        return llvm::ConstantInt::get(CodeGenVisitor::Context, llvm::APInt(bitWidth, val.getInt(),
            !isUnsigned_));
    }
    ...
    llvm::Type* IntegerType::getAllocaType() {
        return Builder->getIntNTy(bitWidth);
    }
    ...
    llvm::Value* IntegerType::castTo(llvm::Value *val, Type *target) {
        switch (target->getTypeID()) {
            ...
            case TypeID::GenericTypeID:
                return Builder->CreateIntToPtr(val, cast<GenericType>(target)->getCovariance()->getLlvmType());
            case TypeID::ObjectTypeID:
            case TypeID::PointerTypeID:
    ...
    string IntegerType::toString() {
        return string(isUnsigned_ ? "U" : "") + "Int" + std::to_string(bitWidth) + Type::toString();
    }
