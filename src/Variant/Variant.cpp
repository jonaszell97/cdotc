...
: type(var.type), bitwidth(var.bitwidth), isUnsigned(var.isUnsigned),
  isBoxed(var.isBoxed), rawStr(var.rawStr), intVal(var.intVal), strVal(var.strVal)
{
...
llvm::Value* Variant::getLlvmValue(CodeGen &CGM) {
   ...
   switch (type) {
      ...
      case VariantType::STRING: {
         return CGM.GetString(strVal, rawStr, true);
      }
