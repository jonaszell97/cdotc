...
{
   return finish(
       sema().getCompilationUnit().getCompilationModule()->getILModule());
...
il::GlobalVariable* ILGenPass::DeclareGlobalVariable(VarDecl* decl)
{
   ...
   auto G
       = Builder.CreateGlobalVariable(decl->getType(), decl->isConst(), nullptr,
                                      OS.str(), decl->getSourceLoc());
...
il::Value* ILGenPass::visitIdentifierRefExpr(IdentifierRefExpr* Expr)
{
   ...
   case IdentifierKind::Function: {
      ...
      if (!Expr->getExprType()->isThinFunctionTy()) {
         ...
         //
         //            V =
         //            Builder.CreateIntrinsicCall(Intrinsic::virtual_method,
         //                                            { Self, Offset });
   ...
   case IdentifierKind::Field: {
      ...
      V = Builder.CreateFieldRef(
          ParentVal, Expr->getDeclName(),
          !Expr->getExprType()->isMutableReferenceType());

      //      if (!Expr->isLHSOfAssignment()) {
      //         SourceLocation EndLoc = Expr->getSourceRange().getEnd();
      //         // mutably borrow this property for the duration of the
      //         property
      //         // access
      //         auto Borrow = Builder.CreateBeginBorrow(
      //            V, Expr->getSourceLoc(), EndLoc,
      //            // a mutable borrow is only needed if we assign to this
      //            property Expr->isLHSOfAssignment());
      //
   ...
   case IdentifierKind::PartiallyAppliedMethod: {
      ...
      auto lambdaTy
          = SP.getContext().getLambdaType(fn->getType()->asFunctionType());
...
il::Value* ILGenPass::visitCallExpr(CallExpr* Expr, il::Value* GenericArgs)
{
   ...
   case CallKind::GenericInitializerCall: {
      ...
      //         CastKind::BitCast, ParamVal,
      //         Context.getMetaType(Expr->getExprType()));
      //
      //      ParamVal = Builder.CreateLoad(ParamVal);
      //
      //      auto *Size = Builder.CreateFieldRef(ParamVal,
      //      SP.getIdentifier("stride")); auto *Init = getMethod(MetaParam,
      //      cast<MethodDecl>(CalledFn)); Init = Builder.CreateBitCast(
      //         CastKind::BitCast, Init, Context.getFunctionType(VoidTy,
      //         {Int8PtrTy}));
      //
      ...
      //
      //      V = Builder.CreateExistentialInit(Alloc, Expr->getExprType(),
      //      ParamVal,
...
void ILGenPass::visitReturnStmt(ReturnStmt* Stmt)
{
   if (Stmt->isFallibleInitReturn()) {
      ...
      auto OptVal = &Builder.GetInsertBlock()
                         ->getParent()
                         ->getEntryBlock()
                         ->getArgs()
                         .front();
      ...
      auto S = dyn_cast<StructDecl>(OptVal->getType()
                                        ->getRecord()
                                        ->getTemplateArgs()
                                        .front()
                                        .getType()
...
il::Value* ILGenPass::visitArrayLiteral(ArrayLiteral* Arr)
{
   ...
   auto Val = Builder.CreateStructInit(ArrDecl, getFunc(Init), {carray, Size});
...
void ILGenPass::initStringInfo()
{
   ...
   StrInfo.StringStorage = cast<StructDecl>(SP.getBuiltinDecl("StringStorage"));
...
il::Constant* ILGenPass::MakeStdString(llvm::StringRef Str)
{
   ...
   if (StrInfo.String->isInvalid() || StrInfo.StringBuffer->isInvalid()
