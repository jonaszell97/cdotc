...
ILWriter::ILWriter(ASTWriter& Writer, llvm::BitstreamWriter& Stream,
                   Context& ILCtx)
...
void ILWriter::WriteValueOffsets(unsigned Offset)
{
   ...
   RecordData::value_type Record[] = {IL_VALUE_OFFSETS, Offsets.size(), 1};
   Stream.EmitRecordWithBlob(Writer.ValueOffsetAbbrev, Record, bytes(Offsets));
}
...
public:
   ...
   EmitKeyDataLength(llvm::raw_ostream& Out, key_type_ref Key, data_type_ref)
   {
   ...
   static void EmitData(llvm::raw_ostream& Out, key_type_ref, data_type Data,
                        unsigned DataLen)
   {
      using namespace llvm::support;
...
void ILWriter::writeFunction(const il::Function& F)
{
   ...
   bool WriteDefinition = ExternallyVisibleFunctions.find(&F)
                          != ExternallyVisibleFunctions.end();
...
void ILWriter::writeInstruction(const il::Instruction& I)
{
   ...
   case Value::BinaryOperatorInstID:
      Writer.push_back(
          static_cast<uint64_t>(cast<BinaryOperatorInst>(I).getOpCode()));

      break;
   case Value::UnaryOperatorInstID:
      Writer.push_back(
          static_cast<uint64_t>(cast<UnaryOperatorInst>(I).getOpCode()));

      break;
   case Value::CompInstID:
      Writer.push_back(static_cast<uint64_t>(cast<CompInst>(I).getOpCode()));
