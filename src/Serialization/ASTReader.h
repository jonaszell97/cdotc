...
private:
   ...
   llvm::DenseMap<const ast::DeclContext *,
                  reader::DeclContextLookupTable> Lookups;
...
private:
   ...
   void addPendingDeclContextInfo(ast::Decl *D,
                                  unsigned SemaDC,
                                  unsigned LexicalDC) {
      assert(D);
...
public:
   ...
   void ReadAttributes(ASTRecordReader &Record,
...
public:
   ...
   ASTRecordReader(ASTReader *Reader,
                   unsigned int Idx,
                   RecordData &&Record)
   ...
   unsigned readDeclID()
   {
      return Reader->ReadDeclID(Record, Idx);
   }
   ...
   lex::Token readToken()
   {
      return Reader->ReadToken(Record, Idx);
   }
