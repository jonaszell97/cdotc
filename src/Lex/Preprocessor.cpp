...
public:
   ...
   PreprocessorImpl(TokenVec& dst, IdentifierTable& Idents,
                    ...
                    llvm::MutableArrayRef<Token> spelledTokens, size_t sourceId)
...
private:
   ...
   struct LispMacro {
      struct Fragment {
         enum Kind { SequencePoint, Variable };

         explicit Fragment(Token tok) : kind(SequencePoint), tok(tok) {}
         ...
         Fragment(Fragment&& F) noexcept : kind(F.kind)
         {
            if (isSequencePoint())
      ...
      public:
         ...
         bool hasNext() { return currentState + 1 < pattern.size(); }
      ...
      private:
         ...
         Fragment const& current() { return pattern[currentState]; }
      ...
      LispMacro(PatternVec&& pattern, TokenVec&& expansionTokens)
          : pattern(std::move(pattern)),
            expansionTokens(std::move(expansionTokens))
      {
      }

      const PatternVec& getPattern() const { return pattern; }

      const TokenVec& getExpansionTokens() const { return expansionTokens; }

      StateMachine makeStateMachine() const { return StateMachine(pattern); }
   ...
   Token const& currentTok() const { return tokens[tokenIndex]; }
   ...
   bool expect(tok::TokenType kind, bool ignoreNewline = true,
   ...
   Token const& lookahead(bool ignoreNewline = true,
                          bool ignoreWhitespace = true, unsigned offset = 1)
   {
   ...
   template<class... Args> void processUntil(Args... kinds)
   {
      while (!currentTok().oneOf(tok::eof, kinds...)) {
         ...
         //            auto macroName =
         //            currentTok().getIdentifier().drop_front(1); auto it =
         //            LispMacros.find(macroName);
         //
   ...
   Token VariantToToken(Variant&& V, SourceLocation loc)
   {
      ...
      default:
         Diags.Diag(err_generic_error)
             << "cannot paste value of type " + variantTypeToString(V) << loc;
      ...
      case VariantType::Int: {
         ...
         return Token((const char*)Mem, Str.size(), tok::integerliteral, loc);
      }
      case VariantType::Floating: {
         llvm::SmallString<32> Str;
   ...
   {
      switch (currentTok().getKind()) {
      case tok::pound_define:
         return handle_define();
      case tok::pound_macro:
         return parse_lisp_macro();
      case tok::pound_undef:
         return handle_undef();
      case tok::pound_if:
         return handle_plain_if();
      case tok::pound_ifdef:
         return handle_ifdef();
      case tok::pound_ifndef:
      ...
      case tok::pound_elseif:
         ...
         break;
      case tok::pound_pragma:
         return handle_pragma();
      case tok::pound_for:
         return handle_for();
      case tok::pound_endfor:
         Diags.Diag(err_generic_error)
             << "#endfor without preceding #for" << currentTok().getSourceLoc();

         break;
      case tok::pound_print:
         return handle_print();
      case tok::pound_let:
         return handle_let();
      case tok::pound_using:
         return handle_using();
      case tok::pound_namespace:
         return handle_namespace();
      case tok::pound_endnamespace:
         return handle_endnamespace();
      case tok::pound_include:
   ...
   template<class... Args> void discard_until(Args... kinds)
   {
      while (!currentTok().oneOf(tok::eof, kinds...))
         advance();
   }

   void skipToEndOfDirective() { discard_until(tok::newline); }
   ...
   void expand_lisp_macro(LispMacro& Macro, llvm::StringRef macroName,
...
void PreprocessorImpl::handle_plain_if()
{
   ...
   if (!V.isInt()) {
      Diags.Diag(err_generic_error)
          << "condition must be integral" << currentTok().getSourceLoc();
   }
   else {
...
void PreprocessorImpl::handle_for()
{
   ...
   if (!arr.isArray()) {
      Diags.Diag(err_generic_error)
          << "value is not an array" << currentTok().getSourceLoc();
...
void PreprocessorImpl::parse_lisp_macro()
{
   ...
   while (!currentTok().is(tok::newline)) {
      if (currentTok().is(tok::percent_ident)) {
         ...
         pattern.emplace_back(
             currentTok().getIdentifierInfo()->getIdentifier());

         variables.insert(pattern.back().getVarName());
      }
      else if (currentTok().is(tok::percent_percent_ident)) {
         Diags.Diag(err_generic_error) << "%% may not appear in macro pattern"
                                       << currentTok().getSourceLoc();
      }
   ...
   if (!currentTok().is(tok::space)) {
      LispMacros.try_emplace(macroName, std::move(pattern),
                                         ...
                                         SourceLocation beginLoc)
{
   ...
   while (SM.hasNext()) {
      ...
      if (currentTok().is(tok::eof)) {
         ...
         Diags.Diag(note_generic_note)
             << "expanding macro " + macroName + " here" << beginLoc;
      }

      if (!SM.moveNext(currentTok())) {
         Diags.Diag(err_generic_error) << "unexpected token in macro expansion"
                                       << currentTok().getSourceLoc();
   ...
   for (const auto& tok : Macro.getExpansionTokens()) {
      if (tok.is(tok::percent_ident)) {
         ...
         tokens.emplace_back(
             VariantToStringLiteral(Variant(move(s)), tok.getSourceLoc()));
      }
      else if (tok.is(tok::space)
               && (tokens.empty() || tokens.back().is(tok::newline))) {
         tokens.emplace_back(Token::Space, tok.getNumSpaces() + baseIndent,
                             tok.getSourceLoc());
      }
      else {
...
#define EXPECT_NUM_ARGUMENTS(ArgNo)                                            \
   if (args.size() != ArgNo) {                                                 \
      Diags.Diag(err_generic_error)                                            \
          << "expected " #ArgNo " arguments, but found "                       \
...
#define EXPECT_ARG_TYPE(ArgNo, ArgTy)                                          \
   if (args[ArgNo].getKind() != VariantType::ArgTy) {                          \
      Diags.Diag(err_generic_error)                                            \
          << "expected " #ArgTy " for argument " #ArgNo << loc;                \
      return {};                                                               \
   }
...
Preprocessor::~Preprocessor() { delete pImpl; }

void Preprocessor::doPreprocessing() { pImpl->doPreprocessing(); }
