...
static constexpr uint16_t DependentMask =
   GenericMask
   | TypeProperties::ContainsAssociatedType;
...
public:
   InstantiatorImpl(SemaPass &SP,
                    ...
                    DeclContext *InstCtx = nullptr,
                    SourceLocation POI = {},
                    QualType Self = QualType())
      : ASTTypeVisitor(false), TypeBuilder(SP, SourceRange(POI)),
   ...
   InstantiatorImpl(SemaPass &SP,
      ...
      : ASTTypeVisitor(false), TypeBuilder(SP, SourceRange()),
   ...
   void instantiateConstraints(NamedDecl *D, NamedDecl *Inst)
   {
      ...
      for (auto *DC : *constraints) {
         ...
         if (DC->getKind() == DeclConstraint::Concept) {
            ...
            constraintInst = DeclConstraint::Create(Context, DC->getKind(),
                                                    instantiatedType,
                                                    DC->getType());
         }
      ...
      if (isa<NamedDecl>(D)) {
         ...
         case Decl::AssociatedTypeDeclID:
            return !cast<AssociatedTypeDecl>(D)
               ->getDefaultType()->isDependentType();
         default:
            return false;
         }
      }
...
public:
   ...
   struct SubstContextRAII {
      SubstContextRAII(InstantiatorImpl &Inst,
         ...
         : Inst(Inst)
      {
         Inst.SubstContexts.emplace_back(param, declName, index, move(makeSubstExpr));
      }
...
private:
   Decl *getInstantiation(Decl *D)
   {
      ...
      if (it == instantiationMap.end()) {
         ...
         assert(Self && isa<NamedDecl>(D) && "no type to lookup instantiations!");
...
private:
   ...
   class VariadicParamSubstVisitor: public TypeBuilder<VariadicParamSubstVisitor> {
      InstantiatorImpl &instantiator;

   public:
      explicit VariadicParamSubstVisitor(InstantiatorImpl &instantiator)
         : TypeBuilder(instantiator.SP, {}),
      ...
      {
         const TemplateArgument *Arg = instantiator.templateArgs.getArgForParam(T->getParam());
         if (!Arg || Arg->isNull()) {
            return T;
         }
   ...
   bool expandVariadicDecl(FieldDecl *Decl)
   {
      ...
      for (unsigned i = 0; i < numVariadics; ++i) {
         ...
         if (auto Ty = Inst->getType().getTypeExpr()) {
            assert(Inst->getType().getTypeExpr() != Decl->getType().getTypeExpr()
   ...
   bool expandVariadicDecl(FuncArgDecl *Decl,
      ...
      for (unsigned i = 0; i < numVariadics; ++i) {
         ...
         auto *Inst = FuncArgDecl::Create(Context, Decl->getOwnershipLoc(),
                                          Decl->getColonLoc(), DN,
                                          Decl->getLabel(),
                                          Decl->getConvention(), T,
                                          copyOrNull(Decl->getDefaultVal()),
                                          false, Decl->isCstyleVararg(),
                                          Decl->isSelf());
...
{
   auto EC = EnumCaseDecl::Create(Context, Decl->getAccess(),
                                  Decl->getCaseLoc(), Decl->getIdentLoc(),
                                  Decl->getDeclName(),
                                  copyOrNull(Decl->getRawValExpr()),
...
PropDecl* InstantiatorImpl::visitPropDecl(PropDecl *Decl, bool IsFieldAccessor)
{
   ...
   auto Prop = PropDecl::Create(Context, Decl->getAccess(),
                                ...
                                visit(Decl->getType()),
                                Decl->isStatic(), Decl->isReadWrite(),
...
Expression* InstantiatorImpl::visitIdentifierRefExpr(IdentifierRefExpr *Ident)
{
   ...
   return new(Context) IdentifierRefExpr(Ident->getSourceRange(),
...
Expression* InstantiatorImpl::visitDeclRefExpr(DeclRefExpr *Expr)
{
   ...
   case Decl::AssociatedTypeDeclID: {
      ...
      NamedDecl *inst;
      if (SP.QC.FindEquivalentDecl(inst, decl,
                                   cast<DeclContext>(SP.getTypeDecl(Self)),
                                   Self)) {
         return nullptr;
      }
...
{
   CanType parentType = Expr->getParentExpr()
                            ->ignoreParensAndImplicitCasts()
                            ->getExprType();

   if (!parentType->containsTemplateParamType() && !parentType->containsAssociatedType()) {
      return MemberRefExpr::Create(Context, visit(Expr->getParentExpr()),
                                   Expr->getMemberDecl(), Expr->getSourceRange());
   }
...
void InstantiatorImpl::instantiateTemplateArgument(const TemplateArgument &TA,
   ...
   if (variadic) {
      list.getArgForParam(TA.getParam())->getVariadicArgs().push_back(move(newArg));
   }
   else {
      ...
      switch (El.getKind()) {
         ...
         case SequenceElement::EF_PossibleOperator: {
            ...
            if (!ident || ident->getIdentInfo() != El.getOp())
               ...
               fragments.emplace_back(El.getOp(), El.getWhitespace(),
                                      El.getLoc());
...
void expandVariadicStmt(InstantiatorImpl &instantiator,
                        ASTContext &Context,
                        NamedDecl *decl,
                        DeclarationName elementName,
                        DeclarationName declName,
   ...
   if (auto *localVar = dyn_cast<LocalVarDecl>(decl)) {
      ...
      isMeta = true;
      param = cast<AliasDecl>(decl)->getType()->removeMetaType()
                                   ->asTemplateParamType()->getParam();
   }
   ...
   for (auto &VA : subst->getVariadicArgs()) {
      ...
      if (isMeta) {
         makeSubstExpr = [&](SourceRange SR) {
            return new(Context) IdentifierRefExpr(
               SR, IdentifierKind::MetaType,
               Context.getMetaType(VA.getType()));
         };
      }
      else {
         DeclarationName instName = Context.getDeclNameTable()
                                           .getPackExpansionName(declName, i);
...
InstantiatorImpl::visitVariadicExpansionExpr(VariadicExpansionExpr *Expr,
   ...
   expandVariadicStmt(*this, Context, Expr->getElementDecl(), declName, declName, [&]() {
...
Statement* InstantiatorImpl::visitVariadicForStmt(StaticForStmt *node)
{
   ...
   if (auto *declRef = dyn_cast<DeclRefExpr>(node->getRange()->getExpr())) {
      ...
      declName = cast<MemberRefExpr>(
         node->getRange()->getExpr())->getMemberDecl()->getDeclName();
   }
...
Decl* InstantiatorImpl::visitVariadicForDecl(StaticForDecl *Decl)
{
   ...
   auto *compoundDecl = CompoundDecl::Create(Context, Decl->getSourceLoc(), true);
...
StaticAssertDecl*InstantiatorImpl::visitStaticAssertDecl(StaticAssertDecl *node)
{
   auto Decl = StaticAssertDecl::Create(Context, node->getStaticAssertLoc(),
                                        node->getParenRange(),
...
{
   auto Decl = StaticPrintDecl::Create(Context, node->getStaticPrintLoc(),
                                       node->getParenRange(),
                                       visit(node->getExpr()));
...
static bool checkDepth(SemaPass &SP,
                       SourceLocation POI,
                       unsigned Depth) {
   unsigned MaxDepth = SP.getCompilationUnit().getOptions().MaxInstantiationDepth;
   if (Depth > MaxDepth) {
      SP.diagnose(err_generic_error,
                  "maximum template instantiation depth (" + std::to_string(MaxDepth) + ") exceeded",
                  POI);
...
static void updateSpecialFunction(QueryContext &QC,
                                  NamedDecl *D,
   ...
   else if (TemplateInfo.IsImplicitlyHashable
         && M == TemplateInfo.HashCodeFn) {
...
QueryResult InstantiateRecordQuery::run()
{
   ...
   if (auto Inst = lookupExternalInstantiation<RecordDecl>(MangledName,
                                                           SP)) {
   ...
   for (auto *ND : Template->getDecls<NamedDecl>()) {
      ...
      if (OuterInst) {
         ...
         if (auto Err = QC.InstantiateTemplateMember(InstResult, ND, Inst,
                                                     TemplateArgs,
                                                     PointOfInstantiation)) {
   ...
   for (auto *Ext : Template->getExtensions()) {
      ...
      Inst->getConformanceTypes().append(SP.Context,
                                         Conformances.begin(),
                                         Conformances.end());
      ...
      for (auto *D : Ext->getDecls<NamedDecl>()) {
         ...
         if (OuterInst) {
            NamedDecl *InstResult;
            if (auto Err = QC.InstantiateTemplateMember(InstResult, D, Inst,
                                                        TemplateArgs,
                                                        PointOfInstantiation)) {
               QueryResult::update(K, Err.K);
            }
   ...
   if (!metaDecls.empty()) {
      ...
      for (auto *metaDecl : metaDecls) {
         ...
         if (auto *compoundDecl = dyn_cast<CompoundDecl>(inst)) {
            for (auto *innerCompoundDecl : compoundDecl->getDecls<CompoundDecl>()) {
               for (auto *innerDecl : innerCompoundDecl->getDecls<NamedDecl>()) {
...
QueryResult InstantiateTemplateMemberQuery::run()
{
   ...
   if (isa<ProtocolDecl>(TemplateMember->getRecord())) {
      NamedDecl *Result;
      if (auto Err = QC.InstantiateProtocolDefaultImpl(Result,
                                              TemplateMember,
                                              QC.Context.getRecordType(Inst),
                                              false)) {
         return Query::finish(Err);
      }
   ...
   if (auto *R = dyn_cast<RecordDecl>(TemplateMember)) {
      if (QC.InstantiateRecord(R, R, TemplateArgs, PointOfInstantiation, Inst)){
         return fail();
      }
      ...
      InstantiatorImpl Instantiator(*QC.Sema, *TemplateArgs,
   ...
   if (TemplateMember->shouldBeSpecialized()) {
      ...
      cast<CallableDecl>(MemberInst)->setBodyTemplate(
...
QueryResult InstantiateMethodBodyQuery::run()
{
   ...
   if (IsExternallyLoaded && isa<MethodDecl>(Inst)) {
      QC.Sema->getILGen()
             .registerInstantiatedImportedMethod(cast<MethodDecl>(Inst));
   }
