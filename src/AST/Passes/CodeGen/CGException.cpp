...
//   bool hasCatches = !node->getCatchBlocks().empty();
//
...
//      (unsigned int)node->getCatchBlocks().size()
//   );
//
//   llvm::BasicBlock *finallyBB = nullptr;
//   llvm::AllocaInst *brAlloca = nullptr;
//
//   if (auto Fin = node->getFinallyBlock()) {
...
//   TryStmt *node)
//{
//   auto hasCatches = !node->getCatchBlocks().empty();
//   auto eh = EHStack.back();
//   EHStack.pop_back();
//
//   auto landingPad = llvm::cast<llvm::LandingPadInst>(
...
//   getResult(node->getFinallyBlock());
//
//   if (!Builder.GetInsertBlock()->getTerminator()) {
//      if (eh.targets.empty()) {
...
//         getTypeInfo(*node->getCatchBlocks()[i].caughtType->getType())
//      );
//
...
//   llvm::Function *descFn = node->getDescFn() ? getOwnDecl(node->getDescFn())
//                                              : nullptr;
//
//   if (!node->getThrownType()->isRefcounted()) {
//      auto size = node->getThrownType()->getSize();
