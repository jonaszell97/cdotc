...
namespace codegen {
   ...
   DebugInfo::DebugInfo(CodeGen &CGM)
      : CGM(CGM), DI(*CGM.Module)
   {
      File = DI.createFile(
         CGM.getFileName(),
         CGM.getPath()
      ...
      const SourceLocation &loc)
   {
      ...
      CGM.Builder.SetCurrentDebugLocation(llvm::DebugLoc::get(
         loc.getLine(),
         loc.getCol(),
         getCurrentScope()
      ));
      ...
      llvm::Instruction *inst)
   {
      ...
      return DI.insertDeclare(
         ...
         CGM.Builder.getCurrentDebugLocation(),
         inst
      );
   }
      ...
      llvm::Value *val)
   {
      ...
      DI.insertDbgValueIntrinsic(
         ...
         DI.createExpression(expr),
         CGM.Builder.getCurrentDebugLocation(),
         &CGM.Builder.GetInsertBlock()->getInstList().back()
      );
   }
...
//      }
   ...
   llvm::MDNode* DebugInfo::emitMethodDI(Method *method, llvm::Function *func)
   {
      if (method->owningClass->isDeclared()) {
         return nullptr;
      }
      ...
      if (method->declaration) {
         ...
         emitParameterDI(method->declaration->getArgs(), func, argBegin, false);
      }

      if (method->declaration && !method->isStatic) {
         auto loc = method->declaration->getSourceLoc();
         auto param = DI.createParameterVariable(
            MD,
            "self",
            1,
