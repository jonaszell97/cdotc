...
public:
   ASTDeserializerImpl(SemaPass &SP,
                       ...
                       module::Module *M,
                       SourceLocation loc)
      : SP(SP), Context(SP.getCompilationUnit().getContext()), M(M), loc(loc),
        Reader(Reader)
   {}
...
#     include "../../AstNode.def"
   ...
   Expression* readExprWithKnownKind(AstNode::NodeType kind)
   {
      ...
      bool hasSubExpr = Reader.ReadByte() != 0;
...
#     include "../../AstNode.def"
      ...
      expr->setSubExpr(std::move(subExpr));
   ...
   ExprSequence::SequenceElement ReadSeqElement()
   {
      ...
      switch (kind) {
         ...
         case ExprSequence::SequenceElement::EF_Operator: {
            auto opKind = (OperatorKind)Reader.ReadULEB128();
            auto loc = ReadSourceLoc();
   ...
   Type *ReadType()
   {
      ...
      switch (typeID) {
         case TypeID::IntegerTypeID: {
            unsigned bw = unsigned(Reader.ReadULEB128());
            bool isUnsigned = (bw & (1 << 10)) != 0;
            bw &= ~(1 << 10);

            return Context.getIntegerTy(bw, isUnsigned);
         }
         case TypeID::FPTypeID:
            return ReadBool() ? Context.getDoubleTy() : Context.getFloatTy();
...
TemplateParamDecl* ASTDeserializerImpl::readTemplateParamDecl()
{
   ...
   auto def = MaybeRead<TemplateArgExpr>();
...
void ASTSerializerImpl::visitTemplateArgExpr(const TemplateArgExpr *stmt)
{
   WriteBool(stmt->isTypeName());
   if (stmt->isTypeName())
...
TemplateArgExpr * ASTDeserializerImpl::readTemplateArgExpr()
{
   auto isTypeName = ReadBool();
   if (isTypeName)
      return new (Context) TemplateArgExpr(readSpecific<TypeRef>());

   return new (Context) TemplateArgExpr(readSpecific<StaticExpr>());
}
...
CaseStmt* ASTDeserializerImpl::readCaseStmt()
{
   ...
   if (isDefault) {
      return new (Context) CaseStmt(readStmt());
   }
   else {
...
RecordDecl *ASTDeserializerImpl::readRecordDecl(AstNode::NodeType kind)
{
   ...
   for (size_t i = 0; i < numDecls; ++i)
      decl->addDecl(readSpecific<NamedDecl>());
      ...
      case TypeRef::ObjectType:
         if (substituteClassName) {
            ...
            for (auto &cont : expr->getNamespaceQual()) {
               WriteString(cont.first);
               WriteList(cont.second, &ASTSerializerImpl::visitTemplateArgExpr);
            }
         }
...
TypeRef* ASTDeserializerImpl::readTypeRef()
{
   ...
   switch (kind) {
      ...
      case TypeRef::ObjectType: {
         ...
         for (size_t i = 0; i < size; ++i) {
            auto name = ReadString();
            auto TAs = ReadList<TemplateArgExpr*>
               (&ASTDeserializerImpl::readTemplateArgExpr);
...
{
   Writer.WriteULEB128(expr->getElements().size());
   for (auto &el : expr->getElements()) {
      WriteString(el.first);
      visitExpr(el.second);
   }
}
...
{
   auto size = Reader.ReadULEB128();
   std::vector<pair<string, Expression* >> elements(size);

   for (size_t i = 0; i < size; ++i) {
      auto name = ReadString();
      auto expr = readExpr();

      elements.emplace_back(move(name), move(expr));
   }
...
void ASTSerializerImpl::visitIdentifierRefExpr(IdentifierRefExpr const* expr)
{
   ...
   WriteList(expr->getTemplateArgs(), &ASTSerializerImpl::visitTemplateArgExpr);
}

IdentifierRefExpr* ASTDeserializerImpl::readIdentifierRefExpr()
{
   auto ident = ReadString();
   auto templateArgs = ReadList<TemplateArgExpr*>
      (&ASTDeserializerImpl::readTemplateArgExpr);

   return new (Context) IdentifierRefExpr(move(ident), move(templateArgs));
}

void ASTSerializerImpl::visitMemberRefExpr(MemberRefExpr const* expr)
{
   WriteBool(expr->isPointerAccess());
   WriteBool(expr->isTupleAccess());

   if (expr->isTupleAccess()) {
      Writer.WriteULEB128(expr->getTupleIndex());
   }
   else {
      WriteString(expr->getIdent());
      WriteList(expr->getTemplateArgs(), &ASTSerializerImpl::visitTemplateArgExpr);
...
MemberRefExpr* ASTDeserializerImpl::readMemberRefExpr()
{
   ...
   auto isTupleAccess = ReadBool();

   if (isTupleAccess) {
      return new (Context) MemberRefExpr(Reader.ReadULEB128(),
                                             isPointerAccess);
   }

   auto MemExpr = new (Context) MemberRefExpr(ReadString(),
                                              isPointerAccess);
...
CallExpr* ASTDeserializerImpl::readCallExpr()
{
   ...
   auto call = new (Context) CallExpr(move(args), move(ident));
...
{
   return new (Context) SubscriptExpr(ReadASTVec<Expression>());
}
...
UnaryOperator* ASTDeserializerImpl::readUnaryOperator()
{
   ...
   return new (Context) UnaryOperator(move(op), move(expr), prefix);
}
...
BinaryOperator* ASTDeserializerImpl::readBinaryOperator()
{
   ...
   return new (Context) BinaryOperator(move(op), move(lhs), move(rhs));
}

void ASTSerializerImpl::visitTertiaryOperator(TertiaryOperator const* expr)
{
...
TertiaryOperator* ASTDeserializerImpl::readTertiaryOperator()
{
   auto cond = readExpr();
   auto lhs = readExpr();
   auto rhs = readExpr();

   return new (Context) TertiaryOperator(move(cond), move(lhs), move(rhs));
}
...
void ASTSerializerImpl::visitExprSequence(ExprSequence const* expr)
{
   ...
   WriteBool(expr->isParenthesized());
}
...
ExprSequence* ASTDeserializerImpl::readExprSequence()
{
   ...
   return new (Context) ExprSequence(move(frags), ReadBool());
}
...
void ASTSerializerImpl::visitLvalueToRvalue(LvalueToRvalue const* expr)
{
   llvm_unreachable("should not be possible");
}

LvalueToRvalue* ASTDeserializerImpl::readLvalueToRvalue()
{
   llvm_unreachable("should not be possible");
}
