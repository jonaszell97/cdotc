...
void Statement::dumpFlags() const { printFlags(llvm::errs()); }
...
void Statement::copyStatusFlags(Statement* Stmt)
{
   ...
   static unsigned StatusFlags = TypeDependent | ValueDependent | HadError
                                 | ContainsGenericParam | ContainsAssociatedType
                                 | ContainsUnexpandedPack | NeedsInstantiation;
...
void Statement::dump() const { print(llvm::outs()); }

DeclStmt::DeclStmt(Decl* D) : Statement(DeclStmtID), D(D) {}

DeclStmt::DeclStmt(EmptyShell Empty) : Statement(DeclStmtID), D(nullptr) {}
...
SourceRange DeclStmt::getSourceRange() const { return D->getSourceRange(); }

AttributedStmt::AttributedStmt(Statement* Stmt, llvm::ArrayRef<Attr*> Attrs)
    : Statement(AttributedStmtID), Stmt(Stmt), NumAttrs((unsigned)Attrs.size())
{
   std::copy(Attrs.begin(), Attrs.end(), getTrailingObjects<Attr*>());
}

AttributedStmt::AttributedStmt(EmptyShell, unsigned N)
    : Statement(AttributedStmtID), Stmt(nullptr), NumAttrs(N)
{
}

AttributedStmt* AttributedStmt::Create(ASTContext& Ctx, Statement* Stmt,
...
AttributedStmt* AttributedStmt::CreateEmpty(ASTContext& C, unsigned N)
{
   void* Mem = C.Allocate(totalSizeToAlloc<Attr*>(N), alignof(AttributedStmt));

   return new (Mem) AttributedStmt(EmptyShell(), N);
}

DebugStmt::DebugStmt(EmptyShell) : Statement(DebugStmtID) {}

NullStmt::NullStmt(SourceLocation Loc) : Statement(NullStmtID), Loc(Loc) {}

NullStmt::NullStmt(EmptyShell) : Statement(NullStmtID) {}
...
CompoundStmt::CompoundStmt(bool preservesScope, SourceLocation LBraceLoc,
                           SourceLocation RBraceLoc)
    : CompoundStmt({}, preservesScope, LBraceLoc, RBraceLoc)
{
}

CompoundStmt::CompoundStmt(llvm::ArrayRef<Statement*> stmts, bool preserveScope,
                           SourceLocation LBraceLoc, SourceLocation RBraceLoc)
    : Statement(CompoundStmtID), numStmts(unsigned(stmts.size())),
      preserveScope(preserveScope), ContainsDeclStmt(false),
...
CompoundStmt::CompoundStmt(EmptyShell, unsigned N)
    : Statement(CompoundStmtID), numStmts(N), preserveScope(false),
      ContainsDeclStmt(false)
{
}

CompoundStmt* CompoundStmt::Create(ASTContext& ASTCtx, bool preserveScope,
...
{
   void* Mem
       = C.Allocate(totalSizeToAlloc<Statement*>(N), alignof(CompoundStmt));
...
BreakStmt::BreakStmt(EmptyShell) : Statement(BreakStmtID) {}
...
ContinueStmt::ContinueStmt(EmptyShell) : Statement(ContinueStmtID) {}
...
IfStmt::IfStmt(SourceLocation IfLoc, ArrayRef<IfCondition> Conditions,
               Statement* body, Statement* elseBody, IdentifierInfo* Label)
    : Statement(IfStmtID), IfLoc(IfLoc), NumConditions(Conditions.size()),
      ifBranch(body), elseBranch(elseBody), Label(Label)
{
...
IfStmt::IfStmt(EmptyShell, unsigned N)
    : Statement(IfStmtID), NumConditions(N), ifBranch(nullptr),
      elseBranch(nullptr)
{
}
...
WhileStmt::WhileStmt(SourceLocation WhileLoc, ArrayRef<IfCondition> Conditions,
                     Statement* body, IdentifierInfo* Label, bool atLeastOnce)
    : Statement(WhileStmtID), WhileLoc(WhileLoc),
      NumConditions((unsigned)Conditions.size()), body(body),
      atLeastOnce(atLeastOnce), Label(Label)
{
   std::copy(Conditions.begin(), Conditions.end(),
             getTrailingObjects<IfCondition>());
}
...
WhileStmt::WhileStmt(EmptyShell, unsigned N)
    : Statement(WhileStmtID), NumConditions(N), body(nullptr),
      atLeastOnce(false)
{
}
...
WhileStmt* WhileStmt::Create(ASTContext& C, SourceLocation WhileLoc,
                             ArrayRef<IfCondition> Conditions, Statement* body,
                             IdentifierInfo* Label, bool atLeastOnce)
{
   void* Mem = C.Allocate(totalSizeToAlloc<IfCondition>(Conditions.size()),
                          alignof(WhileStmt));
...
ForStmt::ForStmt(SourceLocation ForLoc, Statement* init, Expression* term,
                 Statement* inc, Statement* body, IdentifierInfo* Label)
    : Statement(ForStmtID), ForLoc(ForLoc), initialization(init),
      termination(term), increment(inc), body(body), Label(Label)
{
}

ForStmt::ForStmt(EmptyShell)
    : Statement(ForStmtID), initialization(nullptr), termination(nullptr),
      increment(nullptr), body(nullptr)
{
}

ForStmt* ForStmt::Create(ASTContext& C, SourceLocation ForLoc, Statement* init,
...
ForInStmt::ForInStmt(SourceLocation ForLoc, Decl* decl, Expression* range,
                     Statement* body, IdentifierInfo* Label)
    : Statement(ForInStmtID), ForLoc(ForLoc), decl(decl), rangeExpr(range),
      body(body), Label(Label)
{
}

ForInStmt::ForInStmt(EmptyShell)
    : Statement(ForInStmtID), decl(nullptr), rangeExpr(nullptr), body(nullptr)
{
}

ForInStmt* ForInStmt::Create(ASTContext& C, SourceLocation ForLoc, Decl* decl,
...
CaseStmt::CaseStmt(SourceLocation CaseLoc, PatternExpr* pattern,
                   Statement* body)
    : Statement(CaseStmtID), CaseLoc(CaseLoc), pattern(pattern), body(body)
{
}

CaseStmt::CaseStmt(EmptyShell)
    : Statement(CaseStmtID), pattern(nullptr), body(nullptr)
{
}

CaseStmt* CaseStmt::Create(ASTContext& C, SourceLocation CaseLoc,
...
MatchStmt::MatchStmt(SourceLocation MatchLoc, SourceRange Braces,
                     Expression* switchVal, llvm::ArrayRef<CaseStmt*> cases,
                     IdentifierInfo* Label)
    : Statement(MatchStmtID), MatchLoc(MatchLoc), Braces(Braces),
...
MatchStmt::MatchStmt(EmptyShell, unsigned N)
    : Statement(MatchStmtID), switchValue(nullptr), NumCases(N),
      HasDefault(false), HasMutableCaseArg(false), IntegralSwitch(false)
{
}

MatchStmt* MatchStmt::Create(ASTContext& C, SourceLocation MatchLoc,
...
{
   void* Mem = C.Allocate(totalSizeToAlloc<CaseStmt*>(N), alignof(MatchStmt));

   return new (Mem) MatchStmt(EmptyShell(), N);
}

ReturnStmt::ReturnStmt(SourceLocation RetLoc, Expression* val)
    : Statement(ReturnStmtID), RetLoc(RetLoc), returnValue(val)
{
}
...
ReturnStmt* ReturnStmt::Create(ASTContext& C, SourceLocation RetLoc,
                               Expression* val)
{
...
DiscardAssignStmt::DiscardAssignStmt(SourceLocation UnderscoreLoc,
    ...
    : Statement(DiscardAssignStmtID), UnderscoreLoc(UnderscoreLoc),
      EqualsLoc(EqualsLoc), RHS(RHS)
{
}

DiscardAssignStmt::DiscardAssignStmt(EmptyShell)
    : Statement(DiscardAssignStmtID), UnderscoreLoc(), EqualsLoc(), RHS(nullptr)
{
}
...
DoStmt::DoStmt(SourceRange SR, Statement* body,
               ArrayRef<CatchBlock> catchBlocks, IdentifierInfo* Label)
...
ThrowStmt::ThrowStmt(SourceLocation ThrowLoc, Expression* thrownVal)
    : Statement(ThrowStmtID), ThrowLoc(ThrowLoc), thrownVal(thrownVal),
      thrownType(nullptr)
{
}

ThrowStmt::ThrowStmt(EmptyShell) : Statement(ThrowStmtID), thrownVal(nullptr) {}
...
MixinStmt::MixinStmt(EmptyShell) : Statement(MixinStmtID), Expr(nullptr) {}

MixinStmt* MixinStmt::Create(ASTContext& C, SourceRange Parens,
                             Expression* Expr)
{
   return new (C) MixinStmt(Parens, Expr);
}

StaticIfStmt::StaticIfStmt(SourceLocation StaticLoc, SourceLocation IfLoc,
                           StaticExpr* condition, Statement* ifBranch,
                           Statement* elseBranch)
    : Statement(StaticIfStmtID), StaticLoc(StaticLoc), IfLoc(IfLoc),
...
StaticIfStmt::StaticIfStmt(EmptyShell)
    : Statement(StaticIfStmtID), condition(nullptr), ifBranch(nullptr),
      elseBranch(nullptr), Template(nullptr)
{
}

StaticIfStmt* StaticIfStmt::Create(ASTContext& C, SourceLocation StaticLoc,
                                   SourceLocation IfLoc, StaticExpr* condition,
                                   Statement* ifBranch, Statement* elseBranch)
{
   return new (C)
       StaticIfStmt(StaticLoc, IfLoc, condition, ifBranch, elseBranch);
}
...
StaticForStmt::StaticForStmt(SourceLocation StaticLoc, SourceLocation ForLoc,
                             IdentifierInfo* elementName, StaticExpr* range,
                             Statement* body)
    : Statement(StaticForStmtID), StaticLoc(StaticLoc), ForLoc(ForLoc),
      elementName(elementName), range(range), body(body), variadic(false)
{
}

StaticForStmt::StaticForStmt(EmptyShell)
    : Statement(StaticForStmtID), range(nullptr), body(nullptr), variadic(false)
{
}

StaticForStmt* StaticForStmt::Create(ASTContext& C, SourceLocation StaticLoc,
                                     SourceLocation IfLoc,
                                     IdentifierInfo* elementName,
                                     StaticExpr* range, Statement* body)
{
...
MacroExpansionStmt::MacroExpansionStmt(SourceRange SR,
    ...
    : Statement(MacroExpansionStmtID), SR(SR), Delim(Delim),
...
MacroExpansionStmt::MacroExpansionStmt(EmptyShell, unsigned N)
    : Statement(MacroExpansionStmtID), Delim(Delimiter::Paren), NumTokens(N),
      ParentExpr(nullptr)
{
}

MacroExpansionStmt* MacroExpansionStmt::Create(ASTContext& C, SourceRange SR,
                                               DeclarationName MacroName,
...
MacroExpansionStmt* MacroExpansionStmt::CreateEmpty(ASTContext& C, unsigned N)
{
