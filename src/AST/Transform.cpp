...
public:
   ...
   bool doReplacement()
   {
      ...
      if (auto E = dyn_cast<Expression>(rep)) {
         E->setSubExpr(cast<Expression>(orig)->getSubExpr());
      }
...
#     include "AstNode.def"
      ...
      auto stmt = cast<Expression>(SP.getParent(orig));
...
private:
   ...
   bool replaceFromForInStmt(ForInStmt *stmt)
   {
      ...
      REPLACE_OTHER(decl, LocalVarDecl)
      REPLACE(body)
   ...
   bool replaceFromTemplateParamDecl(TemplateParamDecl *stmt)
   {
      ...
      REPLACE_OTHER(defaultValue, TemplateArgExpr)

      return false;
   }

   bool replaceFromTemplateArgExpr(TemplateArgExpr *stmt)
   {
      if (stmt->isTypeName()) {
         REPLACE_OTHER(type, TypeRef)
      } else {
   ...
   {
      for (auto &cont : stmt->elements)
         if (cont.second == orig) {
            cont.second = cast<Expression>(rep);
            return true;
         }
   ...
   {
      REPLACE_RANGE_OTHER(templateArgs, TemplateArgExpr)
   ...
   bool replaceFromTertiaryOperator(TertiaryOperator *stmt)
   {
      REPLACE_EXPR(condition)
      REPLACE_EXPR(lhs)
      REPLACE_EXPR(rhs)
   ...
   bool replaceFromLvalueToRvalue(LvalueToRvalue *stmt)
   {
      REPLACE_EXPR(target)
      return false;
   }
