...
BasicBlock::BasicBlock(Function* parent)
    : Constant(BasicBlockID,
               ValueType(parent->getCtx(), parent->getASTCtx().getLabelTy())),
      parent(parent),
      Instructions(parent && !parent->getBasicBlocks().empty()
                       ? std::move(InstList(this, parent->getBasicBlocks()
                                                      .front()
                                                      .getInstructions()
                                                      .getSymTab()))
...
BasicBlock::BasicBlock(const il::BasicBlock& BB, il::Function& F)
    : Constant(BasicBlockID, BB.getType()), parent(&F),
      Instructions(parent && !parent->getBasicBlocks().empty()
                       ? std::move(InstList(this, parent->getBasicBlocks()
                                                      .front()
                                                      .getInstructions()
...
void BasicBlock::dump() const { print(llvm::outs()); }
...
BasicBlock::InstList& BasicBlock::getInstructions() { return Instructions; }
...
{
   Args.push_back(
       new Argument(ValueType(getCtx(), ty), ArgumentConvention::Owned, this));
