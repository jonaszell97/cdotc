...
protected:
   ...
   il::Value* getSubstitution(const il::Value *V)
   {
      return Substitutions[V];
   }

   template <class T>
   T* getSubstitution(const il::Value *V)
   {
...
public:
   explicit InstructionCloner(ILBuilder &Builder)
      : Builder(Builder)
   {

   }
   void addSubstitution(const il::Value &OldVal,
                                           il::Value *NewVal)
   {
   ...
   {
      return addSubstitution(A,
                             Builder.CreateArgument(A.getType(), A.getConvention(),
                                                    nullptr, A.getName(),
                                                    A.getSourceLoc()));
   }
   ...
   {
      return Builder.CreateDealloc(visit(I.getValue()), I.isHeap(), I.getName());
   }
   ...
   il::Value*
   visitExistentialInitInst(const ExistentialInitInst &I)
   {
      return Builder.CreateExistentialInit(visit(I.getTarget()), I.getType(),
                                           I.getValueTypeInfo(),
                                           I.getProtocolTypeInfo(),
                                           I.isPreallocated(), I.getName());
   }
   ...
   {
      return Builder.CreateGEP(visit(I.getVal()), visit(I.getIndex()), I.isLet(),
                               I.getName());
   }
   ...
   il::Value *visitTupleExtractInst(const TupleExtractInst &I)
   {
      return Builder.CreateTupleExtract(
         ...
         support::cast<ConstantInt>(I.getIndex())->getZExtValue(),
         I.isLet(), I.getName());
   }

   il::Value *visitEnumRawValueInst(const EnumRawValueInst &I)
   {
      return Builder.CreateEnumRawValue(visit(I.getOperand(0)),
                                        !I.getType()->isReferenceType(), I.getName());
   }
   ...
   il::Value*
   visitCaptureExtractInst(const CaptureExtractInst &I)
   {
      return Builder.CreateCaptureExtract(I.getIdx()->getZExtValue(), I.getName());
   }
   ...
   il::Value *visitYieldInst(const YieldInst &I)
   {
      return Builder.CreateYield(
         visit(I.getOperand(0)),
         getSubstitution<BasicBlock>(I.getResumeDst()),
   ...
   {
      if (I.getElseBranch()) {
         return Builder.CreateCondBr(visit(I.getCondition()),
                                     getSubstitution<BasicBlock>(I.getTargetBranch()),
                                     getSubstitution<BasicBlock>(I.getElseBranch()),
                                     visit(I.getTargetArgs()), visit(I.getElseArgs()),
                                     I.getName());
      }
   ...
   {
      auto *Sw = Builder.CreateSwitch(visit(I.getSwitchVal()),
                                      getSubstitution<BasicBlock>(I.getDefault()),
                                      I.getName());
   ...
   {
      return Builder.CreateInvoke(visit(I.getCallee()), visit(I.getArgs()),
   ...
   il::Value*
   visitLLVMIntrinsicCallInst(const LLVMIntrinsicCallInst &I)
   {
   ...
   {
      return Builder.CreateBeginBorrow(visit(I.getOperand(0)),
                                       I.getBeginBorrowLoc(), I.getEndBorrowLoc(),
                                       I.isMutableBorrow(), I.getName());
   }

   il::Value *visitEndBorrowInst(const EndBorrowInst &I)
   {
      return Builder.CreateEndBorrow(visit(I.getOperand(0)), I.getEndBorrowLoc(),
                                     I.isMutableBorrow(), I.getName());
   }
   ...
   {
      return Builder.CreateEnumInit(I.getEnumTy(), I.getCase(), visit(I.getArgs()),
                                    I.getName());
   }
   ...
   il::Value*
   visitBinaryOperatorInst(const BinaryOperatorInst &I)
   {
   ...
   il::Value*
   visitExistentialCastInst(const ExistentialCastInst &I)
   {
      return Builder.CreateExistentialCast(visit(I.getOperand(0)),
                                           visit(I.getTargetTypeInfo()),
                                           I.getKind(), I.getType(), I.getName());
   }
