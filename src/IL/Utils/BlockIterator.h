...
public:
   ...
   explicit PredIterator(Ptr ptr)
      : It(ptr->use_begin())
   {
      advanceToNextTerminator();
   }

   PredIterator(Ptr ptr, bool)
      : It(ptr->use_end())
   {
...
public:
   explicit SuccIterator(Ptr BB)
      : TermInst(BB->getTerminator()), idx(0)
   {

   }

   SuccIterator(Ptr BB, bool)
      : TermInst(BB->getTerminator())
   {
      if (TermInst)
